"""Publishing helpers for slice-classifier outputs."""
...
from schemas import SliceClassificationEvent

logger = logging.getLogger(__name__)


class SlicePublisher:
    def __init__(self, cfg: SliceClassifierConfig, redis_client) -> None:
        self.cfg = cfg
    ...
    async def publish(self, output_event: SliceClassificationEvent) -> None:
        ...
        self.redis.hset(
            ...
            mapping=self._encode_hash(
                {
                    "aiopsSliceClassification": payload.get("prediction"),
                    "aiopsSliceClassificationConfidence": payload.get("score"),
                    "aiopsSliceClassificationSeverity": payload.get("severity"),
                    "aiopsSliceClassificationModelVersion": payload.get("modelVersion"),
                    "aiopsSliceClassificationUpdatedAt": payload.get("timestamp"),
                }
            ),
        )
        ...
        if self._influx_write_api is not None:
            try:
                point = (
                    ...
                    .field("confidence", float(payload.get("score") or 0.0))
