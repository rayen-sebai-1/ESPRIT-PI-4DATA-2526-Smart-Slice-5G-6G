...
def preprocess() -> dict:
    ...
    # Split Data
    df_train, df_val, df_test = slice_wise_temporal_split(
    ...
    # Sequences
    X_train, y_train, _ = preprocessor.create_sequences_by_slice(
        df_train, X_train_raw, y_train_raw
    )
    X_val, y_val, _ = preprocessor.create_sequences_by_slice(
        df_val, X_val_raw, y_val_raw
    )
    X_test, y_test, _ = preprocessor.create_sequences_by_slice(
        df_test, X_test_raw, y_test_raw
    )

    print(
        f"[INFO] Sequences -> Train: {X_train.shape} | Val: {X_val.shape} | Test: {X_test.shape}"
    )
