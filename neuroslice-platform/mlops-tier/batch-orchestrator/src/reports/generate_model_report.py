...
def _render_entry(entry: dict) -> Iterable[str]:
    ...
    lines = [
        ...
        "",
        f"- Model family: `{entry.get('model_family') or entry.get('registered_model_family')}`",
        f"- Task type: `{entry.get('task_type') or 'n/a'}`",
        f"- Created at: `{entry.get('created_at')}`",
        f"- MLflow run ID: `{entry.get('mlflow_run_id') or entry.get('run_id') or 'n/a'}`",
        f"- Experiment: `{entry.get('experiment_name') or 'n/a'}`",
        f"- Quality gate: `{entry.get('quality_gate_status')}`",
        f"- Stage: `{entry.get('stage') or 'n/a'}`",
        f"- Promoted: `{entry.get('promoted')}`",
        f"- Promotion decision: `{entry.get('promotion_status') or 'n/a'}`",
        f"- Artifact format: `{entry.get('format') or entry.get('artifact_format')}`",
        f"- Local artifact: `{entry.get('local_artifact_path') or 'n/a'}`",
        f"- Artifact URI: `{entry.get('artifact_uri') or entry.get('mlflow_artifact_uri') or 'n/a'}`",
        f"- ONNX artifact URI: `{entry.get('onnx_uri') or 'n/a'}`",
        f"- ONNX FP16 artifact: `{entry.get('onnx_fp16_uri') or entry.get('onnx_fp16_path') or 'n/a'}`",
        f"- Preprocessor URI: `{entry.get('preprocessor_uri') or 'n/a'}`",
        f"- ONNX export status: `{entry.get('onnx_export_status')}`",
        f"- Reason: {entry.get('quality_gate_reason') or entry.get('reason') or 'n/a'}",
        "",
        "| Metric | Value |",
        "| --- | --- |",
    ]
